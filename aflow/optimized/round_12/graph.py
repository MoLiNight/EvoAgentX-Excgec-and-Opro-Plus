...
import aflow.optimized.round_12.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Review the output to ensure correctness and completeness
        reviewed_solution = await self.review(
            input=solution['response'],
            instruction=r"""
                请审查以下语法纠错结果，并确保其格式正确、内容完整且符合语法规范。若发现任何问题，请修正并重新输出。

                输出要求：
                - 返回一个 JSON 字符串，不应包含 Markdown 格式。
                - 确保所有字段（如 target、edits）均存在且格式正确。
                - 检查 edits 中的每个条目是否包含 src_interval、tgt_interval、src_content、tgt_content、src_tokens、tgt_tokens、error_type、error_severity、error_description。
