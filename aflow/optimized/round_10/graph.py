...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        # Post-process with Review operator to validate output format
        validated_solution = await self.review(input=solution['response'], instruction=r"""
            验证以下 JSON 输出是否符合格式要求，包括字段完整性、数据类型正确性及错误类型匹配性。若不符合，请修正并返回完整 JSON。

            JSON:
            {input}

            要求：
            - 确保所有字段（target, edits）存在且类型正确。
            - edits 中的每一项必须包含 src_interval, tgt_interval, src_content, tgt_content, src_tokens, tgt_tokens, error_type, error_severity, error_description。
            - error_type 必須從指定錯誤類型範圍內選擇。
            - 若發現任何問題，請修正後返回完整 JSON。
        """)
        return validated_solution['response']
