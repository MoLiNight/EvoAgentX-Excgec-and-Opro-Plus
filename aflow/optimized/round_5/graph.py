...
import aflow.optimized.round_5.prompt as prompt_custom
...
class Workflow:
    ...
    async def __call__(self, problem: str):
        ...
        reviewed_solution = await self.review(
            ...
            instruction=r"""
                请检查以下语法纠错结果，并确保其格式正确、内容完整且符合语法规范。若发现任何问题，请修正并重新输出。

                输出要求：
                - 确保输出是有效的 JSON 格式，不含 Markdown。
                - 每个错误项必须包含 src_interval、tgt_interval、src_content、tgt_content、src_tokens、tgt_tokens、error_type、error_severity 和 error_description。
                - 若发现缺失字段或格式错误，请补充或修正。
                - 若发现错误描述不清晰或不符合实际，请重新表述。
